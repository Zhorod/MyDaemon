...
#cap = cv2.VideoCapture('http://192.168.1.215:8000/stream.mjpg')
cap = VideoCapture('http://192.168.1.215:8000/stream.mjpg')
...
while True:
    ...
    # before recognition
    # save a mydameon training raw training image
    # create a time stamp
    now = datetime.datetime.now()
    string_now = now.strftime("%Y") + "-" + now.strftime("%b") + "-" + now.strftime("%d") + "-" + now.strftime("%H") + "-" + now.strftime("%M") + "-" + now.strftime("%S")
    print("Before date and time : ")
    print(string_now)

    # create a filename
    filename = "C:/Local/temp/mdti-" + string_now + "-raw.jpg"
    print(filename)

    # save the raw file with time stamp
    ...
    # after recognition
    # if there are detections or somethiing interesting
    # save a mydameon training raw training image
    # create a time stamp
    now = datetime.datetime.now()
    string_now = now.strftime("%Y") + "-" + now.strftime("%b") + "-" + now.strftime("%d") + "-" + now.strftime(
        "%H") + "-" + now.strftime("%M") + "-" + now.strftime("%S")
    print("After date and time : ")
    print(string_now)

    # create a filename
    filename = "C:/Local/temp/mdti-" + string_now + "-processed.jpg"
    print(filename)
    ...
    # save the processed file with time stamp
    result = cv2.imwrite(filename, cv2.resize(image_np_with_detections, (800, 600)))
    if result == True:
        print("File saved successfully")
    else:
        print("Error in saving file")
